...
function Home() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [data, cats, accs] = await Promise.all([
          getDashboardSummary(trendMonths, selectedMonth, selectedYear),
          getCategories(),
          getAccounts(),
        ]);
        setSummary(data);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowModal(false);
      resetForm();
      const data = await getDashboardSummary(trendMonths, selectedMonth, selectedYear);
      setSummary(data);
    } catch (err) {
  ...
  return (
    <Layout>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1e1b4b 0%, #4c1d95 50%, #6d28d9 100%)', borderRadius: 20, padding: '1.75rem', marginBottom: '1.5rem', color: 'white' }}>
          <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            ...
            <div style={{ fontSize: '2.8rem', fontWeight: 900, letterSpacing: '-1px' }}>${summary.totalBalance.toFixed(2)}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#86efac' }}>+${summary.monthIncome.toFixed(2)}</div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#fca5a5' }}>-${summary.monthExpenses.toFixed(2)}</div>
              <div style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '0.2rem' }}>This month</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
          {ACTION_BUTTONS.map((btn) => (
            <button key={btn.type} onClick={() => { setTransType(btn.type); resetForm(); setShowModal(true); }}
              style={{ background: btn.bg, border: `1.5px solid ${btn.border}`, borderRadius: 14, padding: '1.25rem 0.5rem', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.6rem', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', padding: '1.5rem', marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <span style={{ fontSize: '0.8rem', color: '#7c3aed', fontWeight: 600, cursor: 'pointer' }}>View all →</span>
          </div>
          {summary.recentTransactions.length === 0 ? (
            ...
            summary.recentTransactions.map((t, i) => (
              <div key={t.transactionID} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: i < summary.recentTransactions.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: TYPE_COLORS[t.status]?.bg || '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {t.status === 'income' ? (
                  ...
                  <div>
                    <div style={{ fontWeight: 600, color: '#1e1b4b', fontSize: '0.9rem' }}>{t.description || t.status}</div>
                    <div style={{ color: '#9ca3af', fontSize: '0.75rem' }}>{t.accountName} • {new Date(t.date).toLocaleDateString()}</div>
                  </div>
                </div>
                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: TYPE_COLORS[t.status]?.color || '#374151' }}>
          ...
          {summary.expensesByCategory.length === 0 ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={summary.expensesByCategory} dataKey="amount" nameKey="categoryName" cx="50%" cy="50%" innerRadius={65} outerRadius={100} paddingAngle={3}>
                    {summary.expensesByCategory.map((_, index) => (<Cell key={index} fill={PIE_COLORS[index % PIE_COLORS.length]} />))}
                  </Pie>
        ...
        {/* INCOME LIST */}
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', padding: '1.5rem', marginBottom: '1.5rem' }}>
          <h3 style={{ margin: '0 0 1rem', color: '#1e1b4b', fontSize: '1rem', fontWeight: 700 }}>Income This Month</h3>
          {summary.monthIncomeList.length === 0 ? (
            ...
            <>
              {summary.monthIncomeList.map((item, i) => (
                <div key={item.transactionID} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: i < summary.monthIncomeList.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ width: 42, height: 42, borderRadius: 12, background: '#ecfdf5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg>
                    </div>
                    <div>
                      <div style={{ fontWeight: 600, color: '#1e1b4b', fontSize: '0.9rem' }}>{item.description || 'Income'}</div>
                      <div style={{ color: '#9ca3af', fontSize: '0.75rem' }}>{item.categoryName || 'Uncategorized'} • {new Date(item.date).toLocaleDateString()}</div>
                    </div>
                  </div>
                  <div style={{ fontWeight: 700, color: '#059669', fontSize: '0.95rem' }}>+${item.amount.toFixed(2)}</div>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0 0', marginTop: '0.5rem', borderTop: '2px solid #f3f4f6' }}>
                ...
                <span style={{ fontWeight: 800, color: '#059669', fontSize: '1rem' }}>+${summary.monthIncome.toFixed(2)}</span>
              </div>
            </>
          )}
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', padding: '1.5rem', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.4rem' }}>
              {[1, 3, 6, 12].map(m => (
                <button key={m} onClick={() => setTrendMonths(m)} style={{ padding: '0.3rem 0.65rem', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.78rem', background: trendMonths === m ? '#1e1b4b' : '#f3f4f6', color: trendMonths === m ? 'white' : '#6b7280' }}>{m}M</button>
              ))}
            </div>
          </div>
          <p style={{ margin: '0.25rem 0 1rem', color: '#9ca3af', fontSize: '0.8rem' }}>Income, expenses and balance over the last {trendMonths} month{trendMonths > 1 ? 's' : ''}</p>
          {summary.monthlyTrend.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={summary.monthlyTrend} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                ...
                <Tooltip contentStyle={{ borderRadius: 10, border: '1px solid #e5e7eb', fontSize: '0.85rem' }} formatter={(value) => `$${Number(value).toFixed(2)}`} />
      ...
      {/* TRANSACTION MODAL */}
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 20, padding: '2rem', width: '100%', maxWidth: 460, maxHeight: '90vh', overflowY: 'auto' }}>
            <h3 style={{ margin: '0 0 1.5rem', color: '#1e1b4b', fontWeight: 800, textTransform: 'capitalize' }}>New {transType}</h3>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <select value={formData.accountID} onChange={e => setFormData({ ...formData, accountID: e.target.value })} style={inputStyle}>
                ...
                {accounts.filter(a => transType === 'transfer' ? a.accountType !== 'savings' : true).map(a => (
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button onClick={() => { setShowModal(false); resetForm(); }} style={{ flex: 1, padding: '0.8rem', borderRadius: 12, border: '1.5px solid #e5e7eb', background: 'white', color: '#374151', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleSubmit} disabled={submitting} style={{ flex: 2, padding: '0.8rem', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #1e1b4b, #4c1d95)', color: 'white', fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}>
