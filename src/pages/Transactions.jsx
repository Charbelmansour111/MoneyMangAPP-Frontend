...
function Transactions() {
  ...
  // Get full category path (e.g. Food & Dining > Restaurant)
  ...
  const filtered = transactions
    .filter(t => filter === 'all' || t.transType === filter)
    .filter(t => !search || (t.description || '').toLowerCase().includes(search.toLowerCase()));

  // Overview stats
  const totalIncome = transactions.filter(t => t.transType === 'income').reduce((s, t) => s + t.amount, 0);
  const totalExpenses = transactions.filter(t => t.transType === 'expense').reduce((s, t) => s + Math.abs(t.amount), 0);
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
              <div style={{ fontSize: '0.7rem', opacity: 0.7, textTransform: 'uppercase', letterSpacing: 1, marginBottom: '0.4rem' }}>
                Income
              </div>
              <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#86efac' }}>
                +${totalIncome.toFixed(2)}
              </div>
              <div style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '0.2rem' }}>
                {transactions.filter(t => t.transType === 'income').length} entries
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#fca5a5' }}>
                -${totalExpenses.toFixed(2)}
              </div>
              <div style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '0.2rem' }}>
                {transactions.filter(t => t.transType === 'expense').length} entries
              </div>
            </div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#c4b5fd' }}>
                {transactions.filter(t => t.transType === 'transfer').length}
              </div>
              <div style={{ fontSize: '0.7rem', opacity: 0.6, marginTop: '0.2rem' }}>
                total transfers
              </div>
            ...
            {['all', 'income', 'expense', 'transfer'].map(type => (
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  fontSize: '0.82rem',
                  background: filter === type ? '#1e1b4b' : '#f3f4f6',
        ...
        <div style={{
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((t, i) => {
              const typeInfo = TYPE_COLORS[t.transType] || TYPE_COLORS.income;
              const isTransfer = t.transType === 'transfer';
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1 }}>
                    ...
                    <div>
                      {/* Title = category name or description */}
                  ...
                  >
                    ...
                    {!isTransfer && (
                      <button
                        onClick={() => setDeleteConfirm(t.transactionID)}
                        style={{
      ...
      {showDetailModal && selectedTransaction && (
        ...
        >
          ...
          >
            {/* Header band */}
            <div style={{
              background: TYPE_COLORS[selectedTransaction.transType]?.bg || '#f3f4f6',
              padding: '2rem',
              textAlign: 'center',
              ...
              <div style={{
                ...
                {selectedTransaction.transType === 'income' ? (
              ...
              <div style={{
                fontSize: '2rem',
                fontWeight: 900,
                color: TYPE_COLORS[selectedTransaction.transType]?.color,
              }}>
                {selectedTransaction.transType === 'income' ? '+' : selectedTransaction.transType === 'expense' ? '-' : ''}
                ${Math.abs(selectedTransaction.amount).toFixed(2)}
              </div>
              <div style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '0.25rem', textTransform: 'capitalize' }}>
            ...
            <div style={{ padding: '1.5rem' }}>
              {[
                {
                  label: 'Category',
                  value: getCategoryPath(selectedTransaction.categoryID) || 'Uncategorized',
                },
                {
                  label: 'Description',
                ...
                {
                  label: 'Date',
                  value: new Date(selectedTransaction.date).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                ...
                {
                  label: 'Time',
                  value: new Date(selectedTransaction.date).toLocaleTimeString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit',
                  }),
                },
                {
                  label: 'Currency',
                  value: selectedTransaction.currenciesCode || 'USD',
                },
              ].map((row, i) => (
                <div key={i} style={{
                  display: 'flex',
      ...
      {showAddModal && (
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                {['income', 'expense', 'transfer'].map(type => (
                  <button
                    key={type}
                    onClick={() => setFormData({ ...formData, transType: type, categoryID: '' })}
                    style={{
                      flex: 1,
                      padding: '0.6rem',
                      borderRadius: 10,
                      border: `2px solid ${formData.transType === type ? TYPE_COLORS[type].color : '#e5e7eb'}`,
                      background: formData.transType === type ? TYPE_COLORS[type].bg : 'white',
                      color: formData.transType === type ? TYPE_COLORS[type].color : '#6b7280',
                      fontWeight: 700,
                      fontSize: '0.82rem',
                      cursor: 'pointer',
                      textTransform: 'capitalize',
            ...
            {formData.transType === 'transfer' ? 'From Account' : 'Account'}
            </label>
            {accounts.length === 0 ? (
            <div style={{
                background: '#fef3c7',
               ...
               ⚠️ No accounts found.{' '}
               <span
             onClick={() => { setShowAddModal(false); window.location.href = '/accounts'; }}
         ...
         style={inputStyle}
          ...
          <option value="">Select account...</option>
           {accounts
          .filter(a => formData.transType === 'transfer' ? a.accountType !== 'savings' : true)
          .map(a => (
          <option key={a.accountID} value={a.accountID}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              <button
                onClick={handleAdd}
                disabled={submitting || !formData.amount || !formData.accountID}
                style={{
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <CategoryPicker
                categories={categories}
                transType={editingTransaction.transType}
